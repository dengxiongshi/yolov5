    ...
    for node in model.graph.node:
        if node.op_type == "Concat":
            ...
            for qnode in qnodes:
                ...
                for subnode in model.graph.node:
                    if len(subnode.input) > 0 and subnode.op_type == "QuantizeLinear" and subnode.input[
                        0] in node.input:
                        subconv = find_quantizelinear_conv(model, subnode)
                        match_pairs.append([major, find_quantize_conv_name(model, subconv.input[1])])
...
def run_export(model: Model, opt, save_file=None, dynamic_batch=False, noanchor=False):
    ...
    input_dummy = torch.zeros(1, 3, *imgsz).to(device)

    if save_file:
        f = save_file
    else:
        f = str(opt.weights).replace('.pt', '_qat.onnx')
    # 最后一层
    model.model[-1].concat = True
    grid_old_func = model.model[-1]._make_grid
    model.model[-1]._make_grid = lambda *args: [torch.from_numpy(item.cpu().data.numpy()).to(item.device) for item in
                                                grid_old_func(*args)]
    ...
    if noanchor:
        ...
        # export onnx
        export_onnx(model, input_dummy, f, opset_version=opt.opset,
...
def apply_custom_rules_to_quantizer(model, opt):
    run_export(model, opt, "temp.onnx")

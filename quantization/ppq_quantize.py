...
# ENGINE_PATH      = 'pretrained_weights/yolov5s_trt_lsq.onnx'  # 生成的 Engine 位置
ONNX_PATH        = r"D:\python_work\yolov5\weights\yolov5s.onnx"       # 你的模型位置
ENGINE_PATH      = r"D:\python_work\yolov5\weights"  # 生成的 Engine 位置
CALIBRATION_PATH = r"D:\python_work\yolov5\datasets\coco128\images\val2017"                         # 校准数据集
BATCHSIZE        = 1
w                = 640
h                = 384
...
QSetting.lsq_optimization_setting.collecting_device  = 'cuda'    # 训练过程的缓存数据位置
...
export_ppq_graph(
    qir, platform=PLATFORM,
    graph_save_to=os.path.join(ENGINE_PATH, "INT8.onnx"),
    config_save_to=os.path.join(ENGINE_PATH, "INT8.json"))

from ppq.utils.TensorRTUtil import build_engine, Benchmark, Profiling

build_engine(
    onnx_file=os.path.join(ENGINE_PATH, "INT8.onnx"),
    engine_file=os.path.join(ENGINE_PATH, "INT8.engine"), int8=True,
    int8_scale_file=os.path.join(ENGINE_PATH, "INT8.json"))

Benchmark(os.path.join(ENGINE_PATH, "INT8.engine"))
Profiling(os.path.join(ENGINE_PATH, "INT8.engine"))
...
# ------------------------------------------------------------
ONNX_PATH        = 'pretrained_weights/yolov5s.onnx'       # 你的模型位置
ENGINE_PATH      = 'pretrained_weights/yolov5s_ppq_trt.onnx'  # 生成的 Engine 位置
CALIBRATION_PATH = '/home/lxc/bishe/calib_coco_images'                         # 校准数据集
BATCHSIZE        = 1
EXECUTING_DEVICE = 'cuda'
# create dataloader
imgs = []

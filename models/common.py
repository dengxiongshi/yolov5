...
class DownConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        ...
        # self.conv1 = Conv(c1, c2 // 2, k, s, p, g, act)
        # self.conv2 = Conv(c2 // 2, c2, k, s, p, g, act)
...
class DenseBlock(nn.Module):
    ...
    def forward(self, x):
        # return self.layer(x)
        return self.cv3(torch.cat((self.layer(self.cv1(x)), self.cv2(x)), 1))
...
class C3k(C3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, k=3):
        ...
        self.m = nn.Sequential(*(C2F_Bottleneck(c_, c_, shortcut, g, k=(k, k), e=1.0) for _ in range(n)))
...
class C3k2(C2f):
    ...
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        self.m = nn.ModuleList(
            C3k(self.c, self.c, 2, shortcut, g) if c3k else C2F_Bottleneck(self.c, self.c, shortcut, g) for _ in range(n)
        )

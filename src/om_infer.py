...
def letterbox(im, new_shape=(384, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True, stride=32):
...
def onnx_inf(onnxModulePath, data):
    sess = rt.InferenceSession(onnxModulePath, providers=['TensorrtExecutionProvider', 'CUDAExecutionProvider', 'CPUExecutionProvider'])
    input_name = sess.get_inputs()[0].name
    output_name = sess.get_outputs()[0].name

    pred_onnx = sess.run([output_name], {input_name: data.reshape(1, 3, 384, 640).astype(np.float32)})
...
if __name__ == "__main__":
    onnxModulePath = r"D:\python_work\yolov5\runs\train\yolov5s_xsmall_autoanchpr_conv_half_20240107\weights\AnFang_20240109.onnx"
    IMG_Path = r"E:\downloads\compress\datasets\FEEDS\WI_PRW_SSM_0322_v2\quantity_img\img04606.jpg"
    imgsz = (384, 640)

    img = cv2.imread(IMG_Path)
    img = cv2.resize(img, (640, 384))
    # BIN_Path = "/data/animal_detection/data/bin_img/m1_4_0.bin"
    # im_bin = np.fromfile(BIN_Path, dtype=np.float32).reshape(3, 384, 640) * 255
    # img = im_bin
    ...
    # inference
    sim_outout_path = r"D:\python_work\yolov5\runs\train\yolov5s_xsmall_autoanchpr_conv_half_20240107\weights\img04606_bgr.bin"
    sim_output = np.fromfile(sim_outout_path, dtype=np.float32).reshape(1, 61200, 8)
    output = [sim_output]
    # print(im.shape, pred[0].shape)
    # pred = np.array(pred)

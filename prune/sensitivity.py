    ...
    def __call__(self, model, dataloader, part, sensitivity=None):
        ...
        for num, k in enumerate(rank_modules):
            ...
            for l, r in enumerate(self.ratio):
                self.logger.info(
                    f'pruning {num}/{len(rank_modules)}: {k}, base_loss:{base_loss_total:4f}, base_b:{base_b_total:4f}, base_o:{base_o_total:4f}, base_c:{base_c_total:4f}, ratio:{r}, thres:{thres}')
                ...
                if len(prune_list):
                    ...
                    self.logger.info(
                        f'temp_loss:{temp_loss_total:4f}, temp_b:{temp_b_total:4f}, temp_o:{temp_o_total:4f}, temp_c:{temp_c_total:4f}')
